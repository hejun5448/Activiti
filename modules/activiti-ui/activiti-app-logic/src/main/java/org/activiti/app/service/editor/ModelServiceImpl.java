...
public class ModelServiceImpl implements ModelService {
	...
	protected AbstractModel internalCreateNewModelVersion(Model modelObject, String comment, User updatedBy,
	...
	public void deleteModel(String modelId, boolean cascadeHistory, boolean deleteRuntimeApp) {
		...
		List<ModelHistory> history = modelHistoryRepository
				.findByModelIdAndRemovalDateIsNullOrderByVersionDesc(model.getId());

		// if the model is an app definition and the runtime app needs to be deleted,
		// remove it now
		if (deleteRuntimeApp && model.getModelType() == Model.MODEL_TYPE_APP) {
			String appDefinitionId = appDefinitionService.getDefinitionIdForModelAndUser(model.getId(),
	...
	@Override
	@Transactional
	public ReviveModelResultRepresentation reviveProcessModelHistory(ModelHistory modelHistory, User user,
